...
inferTTree = function(ptree, w.shape=2, w.scale=1, ws.shape=NA, ws.scale=NA,
                      ...
                      startCTree=NA, updateTTree=TRUE, optiStart=2, dateT=Inf,delta_t=NA,verbose=F, isTp = 0, Pi = c(0.5) , dateInitial = 0) {
  ...
  pTTree <- probTTree(ttree$ttree,off.r,off.p,pi,w.shape,w.scale,ws.shape,ws.scale,dateT,delta_t, isTp,Pi, dateInitial )
  ...
  for (i in 1:mcmcIterations) {#Main MCMC loop
    ...
    pTTree2 <- probTTree(ttree2$ttree,off.r,off.p,pi,w.shape,w.scale,ws.shape,ws.scale,dateT,delta_t, isTp, Pi, dateInitial)
    ...
    if (is.na(prop$qr)) {
      print("prop$qr is missing")
    } else if (is.na(pTTree2)) {
      print("pTTree2 is missing")
    } else if (is.na(pPTreeDiff)) {
      print("pPTreeDiff is missing")
    } else if (is.na(pTTree)) {
      print("pTTree is missing")
    } else {
      # Perform the if statement operations
      # ...
    ...
    if (updateOff.r) {
      ...
      pTTree2 <- probTTree(ttree$ttree,off.r2,off.p,pi,w.shape,w.scale,ws.shape,ws.scale,dateT,delta_t, isTp, Pi, dateInitial)
    ...
    if (updateOff.p) {
      ...
      pTTree2 <- probTTree(ttree$ttree,off.r,off.p2,pi,w.shape,w.scale,ws.shape,ws.scale,dateT,delta_t, isTp, Pi, dateInitial)
      ...
      pTTree2 <- probTTree(ttree$ttree,off.r,off.p,pi2,w.shape,w.scale,ws.shape,ws.scale,dateT,delta_t, isTp, Pi, dateInitial)
