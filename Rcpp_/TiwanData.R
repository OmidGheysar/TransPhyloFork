...
# Define specific values from the table
specific_values_m <- list(`2019` = 0.8, `2020` = 0.8, `2021` = 0.75, `2022` = 0.7, `2023` = 0.33)

# Generate the grid and values
result_m <- create_grid_and_values(start_year, end_year, delta_t, default_value, specific_values_m)

# The grid & values array
grid_m <- result_m$grid
values_m <- result_m$values

# Plot the values with specified type and color
plot(grid_m, values_m, type = 'l', col = 'blue', xlab = 'Grid', ylab = 'Values', main = 'Grid vs. Constant Values', xaxt='n', ylim=c(min(values_m) - 0.1, max(values_m) + 0.1))

axis(1, at=seq(floor(min(grid_m)), ceiling(max(grid_m)), by=1), las=2)
abline(h=seq(floor(min(values_m)), ceiling(max(values_m)), by=0.1), col="gray", lty="dotted")
abline(v=seq(floor(min(grid_m)), ceiling(max(grid_m)), by=1), col="gray", lty="dotted")


res_TP_m <- inferTTree(ptree_c4007, mcmcIterations=50000,w.mean=w.mean,w.std=w.std,ws.mean=ws.mean,ws.std=ws.std, dateT=2026, delta_t=0.01, isTp = 1, time_data = result_m$grid, 
                       prob_data = result_m$values, updatePi = FALSE, updateNeg =FALSE)

mcmc_TP_m=convertToCoda(res_TP_m)
effectiveSize(mcmc_TP_m)

par(mfrow=c(1, 1))

plot(res_TP_m)

plot(medTTree(res_TP_m))

a_TP_m=getIncidentCases(res_TP_m,show.plot = T)

res_TPS_m <- inferTTree(ptree_c4007, mcmcIterations=50000,w.mean=w.mean,w.std=w.std,ws.mean=ws.mean,ws.std=ws.std, dateT=2026, delta_t=0.01, isTp = 3, 
                        time_data = result_m$grid, prob_data = result_m$values, updatePi = FALSE, updateNeg =FALSE)


mcmc_TPS_m=convertToCoda(res_TPS_m)
effectiveSize(mcmc_TPS_m)

plot(res_TPS_m)
plot(medTTree(res_TPS_m)) 

# get incident in this example
a_TPS=getIncidentCases(res_TPS_m,show.plot = T)

par(mfrow=c(1, 2))
Med2TP = medTTree(res_TP_m)
Med2TPS = medTTree(res_TPS_m)
plot(medTTree(Med2TP)) 
plot(medTTree(Med2TPS)) 


In_tp_m  = getIncidentCases(res_TP_m,show.plot = T)
In_tps_m  = getIncidentCases(res_TPS_m,show.plot = T)

sum(In_TPS$unsampCases - In_TP$unsampCases)
sum(In_tps_m$unsampCases - In_tp_m$unsampCases)
